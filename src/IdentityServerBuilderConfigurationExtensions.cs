...
namespace Microsoft.Extensions.DependencyInjection
{
    ...
    public static class IdentityServerBuilderConfigurationExtensions
    {
        ...
        {
            builder.AddApiAuthorization<TUser>(o => { });
            return builder;
        }
            ...
            builder.Services.TryAddEnumerable(
                ServiceDescriptor.Singleton<IConfigureOptions<ApiAuthorizationOptions>, ConfigureIdentityResources>(
                    sp =>
                    {
                        ...
                        var effectiveConfig = configuration ?? sp.GetRequiredService<IConfiguration>()
            ...
            IConfiguration configuration)
        {
            builder.ConfigureReplacedServices();
            builder.Services.TryAddEnumerable(
                ServiceDescriptor.Singleton<IConfigureOptions<ApiAuthorizationOptions>, ConfigureSigningCredentials>(
                    sp =>
                    {
                        var logger = sp.GetRequiredService<ILogger<ConfigureSigningCredentials>>();
                        var effectiveConfig = configuration ??
                                              sp.GetRequiredService<IConfiguration>().GetSection("IdentityServer:Key");
